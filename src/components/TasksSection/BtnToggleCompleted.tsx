...
const BtnToggleCompleted: React.FC<Props> = ({ taskCompleted, taskId, isListInView1 }) => {
  ...
  return (
    <Tooltip
      ...
      className={` ${isListInView1 ? 'order-2 mr-0 sm:order-first sm:mr-4' : 'mr-4 order-0'}`}>
      <button
        className={`${taskCompleted ? 'bg-emerald-200 text-emerald-800 ' : 'bg-amber-200 text-amber-800'} rounded-full font-medium`}
