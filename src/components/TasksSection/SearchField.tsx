...
const SearchField: React.FC = () => {
  ...
  useEffect(() => {
    const checkClick = (e: any) => {
      ...
      if (e.target !== searchResultsRef.current && !searchResultsRef.current!.contains(e.target)) {
        setSearchInputValue('');
      }
    };
        ...
        <ul className='absolute bg-slate-100 rounded-md w-full top-14 p-3 divide-y-2 divide-slate-200'>
          {matchedTasks.map(task => (
            <li key={task.id} className='py-2'>
              <Link to='/' className='flex justify-between transition hover:text-rose-500'>
                <span>{task.title}</span> <span className='text-slate-400'>{task.date}</span>
              </Link>
            </li>
          ))}
        </ul>
