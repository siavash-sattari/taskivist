        ...
        <div>
          {creatingADir && (
            <div className='ml-9 mr-4 my-2'>
              ...
              <input type='text' className='inputStyles w-full' id='dir-name' autoFocus />
            </div>
          )}
          <ul>
            {directories.map((dir: string) => (
              <li key={dir}>
                <NavLink
                  to={`/${dir}`}
                  className={`pr-4 pl-9 py-2 block hover:text-rose-600 dark:hover:text-slate-200 ${currentPath === '/' + dir ? classActive : ''}`}>
                  {dir}
                </NavLink>
              </li>
            ))}
