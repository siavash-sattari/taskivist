...
const ItemDirectory: React.FC<{ dir: string; classActive: string }> = ({ dir, classActive }) => {
  ...
  return (
    <>
      ...
      <li className={`flex justify-between pr-4 itemDirectory ${currentPath === '/' + dir ? classActive : ''}`}>
        <NavLink to={`/${dir}`} className={`pr-4 pl-9 py-2 block hover:text-rose-600 dark:hover:text-slate-200 `}>
          {dir}
        </NavLink>
